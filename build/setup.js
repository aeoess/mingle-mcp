...
function plan() {
    ...
    for (const { name, path } of getConfigPaths()) {
        if (!existsSync(path)) {
            out.push({ name, path, exists: false, action: "create" });
            continue;
        }
        try {
...
function apply(change) {
    try {
        ...
        if (existsSync(change.path))
            config = JSON.parse(readFileSync(change.path, "utf-8"));
        else
        ...
        console.log(`  could not write ${change.path}: ${e.message}`);
        return false;
    }
}
async function setup() {
    ...
    for (const c of changes) {
        if (c.action === "already") {
            console.log(`  ${c.name}: already configured, leaving alone`);
            continue;
        }
