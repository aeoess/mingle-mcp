...
async function api(path: string, opts?: RequestInit, timeoutMs = API_TIMEOUT_MS): Promise<any> {
...
(server as any).tool = (name: string, ...rest: any[]) => {
  if (registeringCanonical) return registerRawTool(name, ...rest);
...
server.tool(
  "publish_intent_card",
  "Publish your profile to the Mingle network: what you're looking for and what you can offer. Cards are Ed25519 signed with your persistent identity and expire by themselves. Returns overlaps with other cards immediately. This is the older surface. publish_intent is the current one.",
  {
...
server.tool(
  "rate_connection",
  "Rate a connection you made through Mingle. After an intro is approved and you have met the person, record how it went. Nothing about a rating is shown to the other person and nothing ranks anyone. This is the older surface and the network route behind it is currently unavailable.",
  {
