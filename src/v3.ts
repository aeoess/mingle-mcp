...
// ── Session-pulse state: ~/.mingle/v3-pulse.json ──────────────────────────
// One file, two things: the last-check timestamp and the background-checks
// preference. The preference lives here rather than in a second store so a user
// clearing ~/.mingle clears everything the agent remembers about them, and so
// there is exactly one place to look when asking "what did I agree to".
...
function readPulse(): PulseState {
  if (!existsSync(V3_PULSE_PATH)) return {};
  try { return JSON.parse(readFileSync(V3_PULSE_PATH, "utf-8")) as PulseState; } catch { return {}; }
}
...
export function getBackgroundChecks(): BackgroundChecks | null {
  const v = readPulse().background_checks;
  return v === "on" || v === "off" ? v : null;
}

export function backgroundChecksAllowed(): boolean {
  return getBackgroundChecks() === "on";
}

export function setBackgroundChecks(enabled: boolean, note?: string): PulseState {
  return writePulse({
